...
- The desktop native/surface fork base is mediamp `4aae5fa`; keep its source history and Apache notice in the fork instead of transplanting files here. Air's API remains the only app API. The optional fork-side `mediamp-air` artifact implements this repository's contracts and emits Kotlin 2.1 metadata; mediamp implementation types remain runtime-only. Remaining fork work is the Linux hosted/live test split, complete native-log redaction, GitHub Packages runtime publication, and measured AMD/Intel DMA-BUF work.
