...
For presentation changes, reconstruct the current Roku screen from the pinned visual and behavior specifications together, including dynamic source overrides. Use packaged Roku assets and the canonical 1280×720 frame. Finish an entire requested UI change before integrated builds/tests. Visual acceptance requires matching-content Roku/Android captures inspected privately; behavior tests alone never establish visual parity. The 2026-09-12 presentation was rejected and replaced; old acceptance records are historical only.
